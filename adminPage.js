...
function confirmDelete(automationId, automationName) {
  Swal.fire({
    ...
    html: `Você tem certeza que deseja excluir a automação <br>"${automationName}"?`,
...
//   console.log(startHour, adjustedEndHour);

//   if (parseInt(startHour) === parseInt(adjustedEndHour)) {
//     // Combine the values into a cron expression with adjusted end hour
//     let cronExpression = `${minutes} ${adjustedEndHour} ${dayOfMonth} ${month} ${selectedDays}`;
//     console.log('Generated Cron Expression:', cronExpression);
//   } else {
//     // Combine the values into a cron expression with adjusted end hour
//     let cronExpression = `${minutes} ${startHour}-${adjustedEndHour} ${dayOfMonth} ${month} ${selectedDays}`;
//     console.log('Generated Cron Expression:', cronExpression);
//   }
...
function addCronRule() {
  ...
  const startHour = (document.getElementById('cronStartHour')?.value);
  const endHour = (document.getElementById('cronEndHour')?.value);
  const startMinute = (document.getElementById('cronStartMinute')?.value);
  const endMinute = (document.getElementById('cronEndMinute')?.value);
  ...
  // Validate that end hour is greater than start hour
  if (
    startHour === undefined || startHour === null || startHour === "" ||
    endHour === undefined || endHour === null || endHour === "" ||
    startMinute === undefined || startMinute === null || startMinute === "" ||
    endMinute === undefined || endMinute === null || endMinute === "" ||
    !/^\d+$/.test(startHour) || !/^\d+$/.test(endHour) ||
    !/^\d+$/.test(startMinute) || !/^\d+$/.test(endMinute)
  ) {
  ...
  // Validate limits: Hours must be between 0-23, Minutes must be between 0-59
  if (
    parseInt(startHour) > 23 || parseInt(endHour) > 23 ||
    parseInt(startMinute) > 59 || parseInt(endMinute) > 59
  ) {
    showPopup("As horas devem estar entre 0 e 23, e os minutos entre 0 e 59.");
    return;
  }
  ...
  const formattedStartHour = String(Number(startHour)).padStart(2, '0');
  const formattedStartMinute = String(Number(startMinute)).padStart(2, '0');
  const formattedEndHour = String(Number(endHour)).padStart(2, '0');
  const formattedEndMinute = String(Number(endMinute)).padStart(2, '0');

  const ruleText = `${formattedDays} | ${formattedStartHour}:${formattedStartMinute} até ${formattedEndHour}:${formattedEndMinute}`;
  ...
  ruleItem.dataset.startHour = startHour; // Store adjusted end hour
  ruleItem.dataset.startMinute = startMinute;
  ruleItem.dataset.endHour = endHour;
  ruleItem.dataset.endMinute = endMinute;

  container.appendChild(ruleItem);

  // Clear inputs
  document.getElementById('cronStartHour').value = '';
  document.getElementById('cronStartMinute').value = '';
  document.getElementById('cronEndHour').value = '';
  document.getElementById('cronEndMinute').value = '';
...
function removeCronRule(button) {
  ...
  cronExpression.length = 0; // Clears the array
}
...
function generateCron() {
  ...
  parseInt(document.getElementById('cronStartHour')?.value, 10)
  // Get data from the selected rule
  const days = selectedRule.dataset.days;
  const startHour = parseInt(selectedRule.dataset.startHour, 10);
  const endHour = parseInt(selectedRule.dataset.endHour, 10);
  const startMinute = parseInt(selectedRule.dataset.startMinute, 10);
  const endMinute = parseInt(selectedRule.dataset.endMinute, 10);
  // const displayEndHour = parseInt(endHour); // Convert back to display hour
  ...
  if (cronExpression.length == 0) {
    // Case: 00:00 - 23:59 (Every minute of the day) ✅ Optimized
    if (
      ...
      cronDisplayDescription = `Dias: ${formattedDays} | 00:00 até 23:59`;
      cronExpression.push(`* * * * ${days}`);  // Runs every minute of the day
    }

    // Case: 08:00 - 09:00 (One full hour)
    else if (
      parseInt(startHour) === parseInt(endHour) - 1 &&
      parseInt(startMinute) === 0 &&
      parseInt(endMinute) === 0
    ) {
      cronDisplayDescription = `Dias: ${formattedDays} | ${formattedStartHour}:00 até ${formattedEndHour}:00`;
      cronExpression.push(`* ${startHour} * * ${days}`);
    }

    // Case: 08:00 - 18:00 (Multi-hour, full hour blocks)
    else if (
      parseInt(startMinute) === 0 && parseInt(endMinute) === 0
    ) {
      cronDisplayDescription = `Dias: ${formattedDays} | ${formattedStartHour}:00 até ${formattedEndHour}:00`;
      cronExpression.push(`* ${startHour}-${endHour} * * ${days}`);
    }

    // Case: 08:10 - 18:00 (Non-zero start minute, full-hour end)
    else if (
      parseInt(startMinute) !== 0 && parseInt(endMinute) === 0
    ) {
      cronDisplayDescription = `Dias: ${formattedDays} | ${formattedStartHour}:${formattedStartMinute} até ${formattedEndHour}:00`;
      cronExpression.push(`${startMinute}-59 ${startHour} * * ${days}`);
      cronExpression.push(`* ${startHour + 1}-${endHour - 1} * * ${days}`);
    }

    // Case: 08:00 - 18:10 (Full-hour start, non-zero end minute)
    else if (
      parseInt(startMinute) === 0 && parseInt(endMinute) !== 0 &&
      parseInt(startHour) !== parseInt(endHour)
    ) {
      cronDisplayDescription = `Dias: ${formattedDays} | ${formattedStartHour}:00 até ${formattedEndHour}:${formattedEndMinute}`;
      cronExpression.push(`* ${startHour}-${endHour - 1} * * ${days}`);
      cronExpression.push(`00-${endMinute} ${endHour} * * ${days}`);
    }

    // Same hour (e.g., 23:00 - 23:10) 
    else if (
      parseInt(startHour) === parseInt(endHour)
    ) {
      cronDisplayDescription = `Dias: ${formattedDays} | ${formattedStartHour}:${formattedStartMinute} até ${formattedEndHour}:${formattedEndMinute}`;
      cronExpression.push(`${startMinute}-${endMinute} ${startHour} * * ${days}`);
    }

    // Case: 08:10 - 09:10 (Different hours, same minutes)
    else if (
      parseInt(startHour) !== parseInt(endHour) &&
      parseInt(startMinute) === parseInt(endMinute) &&
      parseInt(endHour) == parseInt(startHour) + 1
    ) {
      cronDisplayDescription = `Dias: ${formattedDays} | ${formattedStartHour}:${formattedStartMinute} até ${formattedEndHour}:${formattedEndMinute}`;
    ...
    // Case: 08:10 - 09:30 (Different hours, different minutes) End hour only +1 than Start hour
    else if (
      parseInt(startHour) !== parseInt(endHour) &&
      parseInt(startMinute) !== parseInt(endMinute) &&
      parseInt(endHour) == parseInt(startHour) + 1
    ...
    // Case: 08:10 - 10:30 (Different hours, different minutes) End hour only +2 than Start hour
    else if (
      parseInt(startHour) !== parseInt(endHour) &&
      parseInt(startMinute) !== parseInt(endMinute) &&
      parseInt(endHour) == parseInt(startHour) + 2
    ) {
      cronDisplayDescription = `Dias: ${formattedDays} | ${formattedStartHour}:${formattedStartMinute} até ${formattedEndHour}:${formattedEndMinute}`;
      cronExpression.push(`${startMinute}-59 ${startHour} * * ${days}`);
      cronExpression.push(`* ${startHour + 1} * * ${days}`);
      cronExpression.push(`00-${endMinute} ${endHour} * * ${days}`);
    }

    // Case: 08:10 - 18:10 (Non-zero start & end minutes, multi-hour)
    else {
      cronDisplayDescription = `Dias: ${formattedDays} | ${formattedStartHour}:${formattedStartMinute} até ${formattedEndHour}:${formattedEndMinute}`;
      cronExpression.push(`${startMinute}-59 ${startHour} * * ${days}`);
      cronExpression.push(`* ${startHour + 1}-${endHour - 1} * * ${days}`);
      cronExpression.push(`00-${endMinute} ${endHour} * * ${days}`);
    } // Ensure cronExpression is an array
  }
...
function clearAutomationForm() {
  ...
  // Reset cron variables (Ensure these are global)
  if (typeof cronExpression !== 'undefined') {
    cronExpression.length = 0; // Clears array while maintaining reference
  }

  if (typeof cronDisplayDescription !== 'undefined') {
    cronDisplayDescription = ''; // Clears description
  }
...
function saveAutomation() {
  ...
  if (message.length > 255) {
    showError('message', 'A mensagem não pode exceder 255 caracteres.');
...
function closeUpdateModal() { // Clear the form
...
function updateAutomation() {
  ...
  if (message.length > 255) {
    showError('updateMessage', 'A mensagem não pode exceder 255 caracteres.');
    errors.push('Mensagem muito longa.');
  }
